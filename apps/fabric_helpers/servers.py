...
class Machine(object):
    def __init__(self, public_address, env_name, short_name=None, private_address=None, 
        ...
        if base_server:
            ...
            self.base_server = BaseUbuntuServer(ssh_port=ssh_port)
    ...
    def setup_config_files(self, project_conf_dir=None, server_type=None):
        if self.conf_files:
            ...
            if not project_conf_dir:
                project_conf_dir = os.path.join(state.env.paths['live'], state.env.project_name, 'config/')
...
class ApacheServer(WebServer):
    ...
    def wsgi_dir(self):
        return '%s/wsgi_handlers' % state.env.paths['live']
